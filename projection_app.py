...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab_a:
        # Toggle between preseason and midseason — not a free slider (avoids post-season redundancy)
        snap_label = st.radio(
            "Projection snapshot", ["Preseason (before Week 1)", "Midseason (before Week 10)"],
            horizontal=True)
        snap_week = PRESEASON_WK if "Preseason" in snap_label else MIDSEASON_WK
        ...
        fig_all.update_layout(
            ...
            title=dict(text=f"{snap_label} — grey bar = 80% CI  ·  diamond = actual final wins",
        ...
        fig_scat.update_layout(
            title=dict(text=f"{snap_label} MAE = {mae_val:.2f} wins", font=dict(color=TEXT_DARK)),
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab_e1:
        ...
        with col2:
            st.subheader("Model vs Vegas Accuracy (2025 Test)")
            models_to_show = {
            ...
            fig_vb.update_layout(
                yaxis=dict(title="Test Accuracy", tickformat=".0%",
                           range=[0.55, 0.73], tickfont=dict(color=SLATE)),
                xaxis=dict(tickfont=dict(color=TEXT_DARK)))
            st.plotly_chart(clean(fig_vb, h=300), use_container_width=True)
            st.caption(f"Vegas spread correctly picks the winner {vegas_acc:.1%} of the time — "
